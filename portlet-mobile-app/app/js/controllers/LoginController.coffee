'use strict'

appController = angular.module('LoginController', ['ngRoute', 'ngStorage' , 'ionic','Api','ngCordova', 'ngCordovaOauth'])

appController.controller(
  'LoginController'
  [
    "$scope"
    "$cordovaOauth" 
    "$location"
    "portletApi"
    "$log"
    "$http"
    "$cookies"
    "$localStorage"
    "$state"
    ($scope, $cordovaOauth, $location, portletApi,$log, $http, $cookies, $localStorage,$state) ->
      console.log("u r in LoginController")
      $scope.login = ->
        $localStorage.accessToken = 'ya29.BgLdfL99ZcN49dTdlySEN5buFoI-IJcQQ3DYVBnm3hdwYjdKoCEWHd1TFOn3-dPgL90t'
        access_token = 'ya29.BgLdfL99ZcN49dTdlySEN5buFoI-IJcQQ3DYVBnm3hdwYjdKoCEWHd1TFOn3-dPgL90t'
        console.log "token From google server is : " + access_token

        portletApi.authGoogle(
          access_token
          {
            before: ->
              $log.debug('authentication.')
            success: (data, status, headers, config) ->
              console.log 'oauth data: ' + data
              window.localStorage['token'] = access_token;
              console.log 'token : ' + window.localStorage['token']
            error: (data, status, headers, config, statusText) ->
              $log.error('Got error while authentication.')
            complete: (data, status, headers, config) ->
              $log.debug('In complete of authentication')
              $location.path '/app/portfolio'
          }
        )
        return
#        for testing on web
#        $cordovaOauth.google('945704286859-9fpaqjv25ce7ofuscp7b62hs5qd0j6b6.apps.googleusercontent.com', [
#          'https://www.googleapis.com/auth/userinfo.email'
#          'https://www.googleapis.com/auth/plus.me'
#          'https://www.googleapis.com/auth/userinfo.profile'
#        ]).then ((result) ->
#          $localStorage.accessToken = result.access_token
#          access_token = result.access_token
#
#          console.log "token From google server is : " + access_token
#
#          portletApi.authGoogle(
#            access_token
#            {
#              before: ->
#                $log.debug('authentication.')
#              success: (data, status, headers, config) ->
#                console.log 'oauth data: ' + data
#                window.localStorage['token'] = access_token;
#                console.log 'token : ' + window.localStorage['token']
#              error: (data, status, headers, config, statusText) ->
#                $log.error('Got error while authentication.')
#              complete: (data, status, headers, config) ->
#                $log.debug('In complete of authentication')
#                $location.path '/app/portfolio'
#            }
#          )
#          return
#        ), (error) ->
#          alert 'There was a problem signing in!  See the console for logs'
#          console.log error
#          return
        return
  ])
